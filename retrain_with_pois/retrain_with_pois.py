...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 19 22:20:27 2020
...
def main():
    ...
    x_test = np.vstack((x_test_clean, x_test_poisoned))
    y_test = np.hstack((y_test_clean, np.zeros(y_test_poisoned.shape)+np.max(y_test_clean)+1))
    ...
    #clean_label_p = np.argmax(bd_model.predict(y_test_poisoned), axis=1)
    #class_accu = np.mean(np.equal(clean_label_p, np.zeros(y_test_poisoned.shape)+1+np.max(y_test_clean)))*100
    #print('Accuracy for poisned data identified as n+1 on retrained:', class_accu)
    
    bd_model.save("./models/retrain_with_pois_sunglasses_bd_net_temp.h5")
